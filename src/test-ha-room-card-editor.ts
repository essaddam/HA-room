...
function testEditorConfiguration() {
  ...
  try {
    ...
    // Test 4: Test assertConfig function
    try {
      configForm.assertConfig({
        name: 'Test Room',
        icon: 'mdi:home',
        icon_color: '#ffffff'
      });
      ...
      throw new Error(`assertConfig should accept valid config: ${error.message}`);
    }
    ...
    try {
      ...
      if (error.message.includes('Le nom de la pièce est requis')) {
        console.log('✅ Test 5 Passed: assertConfig correctly rejects invalid configuration');
      } else {
        throw new Error(`assertConfig should reject empty config with specific message: ${error.message}`);
      }
    }
...
function testHACSCompatibility() {
  ...
  const haRoomCard = window.customCards.find(card => card.type === 'ha-room-card');
  if (!haRoomCard) {
